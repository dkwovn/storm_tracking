...
do i=0,7,2
    ...
    .and.lat(en_y(i))>=lat_min.and.lat(en_y(j))<=lat_max)then
...
function distance_g_deg(lon1_deg, lat1_deg, lon2_deg, lat2_deg)
!=== Note that the lon, lat here are in degree ===
real,intent(in) :: lon1_deg, lon2_deg, lat1_deg, lat2_deg
real :: distance_g_deg
...
distance_g_deg = acos(cos(lat1)*cos(lat2)*cos(lon1-lon2) + sin(lat1)*sin(lat2))
end function distance_g_deg
